...
from tools import CreateDraftTool, search_tool
...
class Agents():
    def email_categorizer_agent(self):
        return Agent(
            role = 'Senior Email Analyst',
            goal = 'Filter out non-essential emails like newsletters and promotional content',
            backstory = dedent("""\
            As a Senior Email Analyst, you have extensive experieince in email content analysis.
            You are adept at distinguishing important emails from spam, newletters, and other irrelevant content.
            Your expertise lies in identifying key patterns and markers that signify the importance of an email."""),
            verbose = True,
            allow_delegation = False
            )
    
    def email_researcher_agent(self):
        return Agent(
            role = 'Email Action Specialist',
            goal = 'Decide what information is needed to search for the email writer to reply in a thoughtful and helpful way based on the category of the email.',
            backstory = dedent("""\
